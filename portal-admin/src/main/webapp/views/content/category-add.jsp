...
<body>
	<div class="content-menu ib-a blue line-x">
		...
		<span>|</span>
		<a href="javascript:;" class="on"> <em>添加栏目</em>
		</a>
	</div>
		...
		<div class="pad-10">
			...
			<div class="col-tab">
				...
				<div id="div_setting_1" class="contentList pad-10">
					<form id="addCategoryForm" class="form-horizontal" action="/content/category/addNew" method="post">
						<div class="control-group">
							<label class="control-label">栏目类型:</label>
							<div class="controls">
								<select class="span2" name="categoryType" id="catType">
									<option value="PAGE" selected>单网页</option>
									<option value="NEWS-CATEGORY" >内容栏目</option>
								</select>
							</div>
						</div>
						<div class="control-group">
							...
							<div class="controls">
								<select class="span2" name="parentCategory" id="parentCat">
									<option value="0" <c:if test="${empty parentCategory}">selected</c:if>>≡ 作为一级栏目 ≡</option>		
									<c:forEach var="select" items="${selectList}">
										<c:if test="${select.level <= 0}">
											<option value="${select.catId}" <c:if test="${parentCategory.catId == select.catId}">selected</c:if>>${select.catName}</option>
										</c:if>
										<c:if test="${select.level > 0}">
											<option value="${select.catId}" <c:if test="${parentCategory.catId == select.catId}">selected</c:if>>
						...
						<div class="control-group">
							...
							<div class="controls">
								<input type="text" id="catName" name="categoryName" value="" maxlength="20" />
						...
						<div class="control-group">
							...
							<div class="controls">
								<input type="text" id="catPath" name="categoryPathName" value="" maxlength="20" />
